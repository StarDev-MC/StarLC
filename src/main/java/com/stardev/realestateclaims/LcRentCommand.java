...
public final class LcRentCommand implements CommandExecutor {
    private final RealEstateClaims plugin;

    public LcRentCommand(RealEstateClaims plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 2) {
            sender.sendMessage(Component.text("Usage: /lcrent <id> <price>").color(net.kyori.adventure.text.format.NamedTextColor.YELLOW));
            return true;
        }
        int id;
        double price;
        ...
        claim.setRentPrice(price);
        plugin.getClaimManager().updateClaim(claim);
        plugin.getClaimManager().updateSign(claim);
        sender.sendMessage(Component.text("Claim #" + id + " rent price set to $" + price + " (daily).").color(net.kyori.adventure.text.format.NamedTextColor.GREEN));
        return true;
    }
}
