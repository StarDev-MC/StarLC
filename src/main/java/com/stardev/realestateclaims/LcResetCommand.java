...
public final class LcResetCommand implements CommandExecutor {
    private final RealEstateClaims plugin;

    public LcResetCommand(RealEstateClaims plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage(Component.text("Usage: /lcreset <id|all>").color(net.kyori.adventure.text.format.NamedTextColor.YELLOW));
            return true;
        }
        if (args[0].equalsIgnoreCase("all")) {
            ...
            sender.sendMessage(Component.text("Reset " + total + " claims to default state.").color(net.kyori.adventure.text.format.NamedTextColor.GREEN));
            return true;
        }
        int id;
        try {
        ...
        plugin.getClaimManager().updateSign(claim);
        sender.sendMessage(Component.text("Claim #" + id + " has been reset to default state.").color(net.kyori.adventure.text.format.NamedTextColor.GREEN));
        return true;
    }
}
